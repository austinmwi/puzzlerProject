...
public class ProceduralAudio : MonoBehaviour
{
	...
	public double doppler_effect								= 128;		//the amount of pitch shift for moving sounds (hack)
	...
	private double _celerity								= 0.0;		//this is the "group" volume of the sound
	...
	private double _gain_master							= 0.0125;	//the master volume adjustment
	private double _gain_left							= 0.0;		//left channel gain adjustment
	private double _gain_right							= 0.0;		//right channel gain adjustment

	private Vector3 _position_of_sound 					= Vector3.zero;
	private Vector3 _direction_to_sound					= Vector3.zero;
	private Vector3 _direction_of_left_ear				= Vector3.zero;
	private Vector3 _direction_of_right_ear				= Vector3.zero;
	private double 	_distance_to_sound					= 0.0;
	private double 	_distance_to_sound_last_frame 		= 0.0;
	...
	private void SetGain()
	{
		...
		float scale						= 1.5f;
		_direction_of_left_ear			= Vector3.Normalize(gameObject.transform.forward - gameObject.transform.right * scale);
		_direction_of_right_ear			= Vector3.Normalize(gameObject.transform.forward + gameObject.transform.right * scale);
		...
		_gain_left						= Vector3.Dot(_direction_to_sound, _direction_of_left_ear)  * gain + gain;
		_gain_right						= Vector3.Dot(_direction_to_sound, _direction_of_right_ear) * gain + gain;
	}


	//simple doppler effect pitch shifting
	private void SetCelerity()
	{
		...
		_celerity						= Lerp(_distance_to_sound_last_frame - _distance_to_sound, _celerity, 0.125);
	}
	...
	private void SetTheta()
	{
		...
		_theta 							= ((frequency + _celerity * doppler_effect) * 2 * Math.PI) / _sample_rate;
	...
	{
		for (var i = 0; i < buffer.Length; i = i + channels)
		{
			//generate the wave amplitude at this position
			_amplitude	= Math.Sin(_phase);


			// if we have stereo, generate the audio and adjust the gain per ear
			if (channels == 2)
			{
				buffer[i]		= (float)(_gain_master * _gain_left  * Math.Sin(_amplitude));
				buffer[i + 1]	= (float)(_gain_master * _gain_right * Math.Sin(_amplitude));
			}
			else
			{
				buffer[i]		= (float)(_gain_master * _phase);
			}
