	...
	SubShader 
	{
		...
		{ 
			...
			float map(float3 position)
			{
				...
				position.xz				= mul(rmat(time*.0025), position.xz);

				float wavesb			= voronoi(5. - position.xz * _Frequency.y + time * _Velocity.y) * _Scale.y;
			
				float waves				= lerp(wavesa, wavesb, _Scale.z);
...
//				o.color.xyz				*= 0.; o.color.xyz				+= rays;
			...
			fixed4 frag (v2f i) : COLOR
			{
				...
				if(_Reflection > 0.)
				{
					...
					v.y					+= map(i.normal + i.position.xyz * 8. + _Time.x) * 8.;
